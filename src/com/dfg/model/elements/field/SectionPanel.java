...
import com.dfg.model.elements.LocalizedString;
...
public class SectionPanel extends AbstractFormElementPanel {
	...
	public SectionPanel(final String id, final SectionModel model,
	...
	protected void onConfigure() {
		if (!hasBeenRendered()) {
			...
			Fragment fragment;
			if (model.isHidden()) {
				fragment = new Fragment("sectionFragment", "hiddenSection",
						this);
			} else {
				fragment = new Fragment("sectionFragment", "defaultSection",
						this);
				Label titleLabel = new Label("sectionTitle", model.getTitle());
				fragment.add(titleLabel);
			}

			ListView<AbstractFormElementModel> elementList = createElementList(
					"elementList", model);
			fragment.add(elementList);
			removeButton = createAjaxButton("removeButton");
			removeButton.setDefaultFormProcessing(false);
			removeButton.add(new AttributeModifier("value",
					getString(LocalizedString.REMOVE_BUTTON_LABEL
							.getResourceKey())));
...
/**
	 ...
	 */
	private ListView<AbstractFormElementModel> createElementList(
			final String wicketId, final SectionModel section) {

		IModel<List<AbstractFormElementModel>> model = new PropertyModel<List<AbstractFormElementModel>>(
		...
		listView = new ListView<AbstractFormElementModel>(wicketId, model) {
			...
			protected void populateItem(
					final ListItem<AbstractFormElementModel> item) {
	...
	private <T> Component createFormElementPanel(final String wicketId,
