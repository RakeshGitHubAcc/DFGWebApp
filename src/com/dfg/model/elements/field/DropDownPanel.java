...
public class DropDownPanel<T> extends AbstractInputFieldPanel<T> {
	...
	public DropDownPanel(final String id, final DropDownModel<T> model) {
		...
		this.dropdown = new DropDownChoice<T>("inputField",
				new PropertyModel<T>(model, "value"), model.getChoices());
		this.decorateComponent(this.dropdown);
		this.dropdown.setChoiceRenderer(WickedFormsChoiceRenderer
